"""
train_hust.py — Training Script for HUST Motor Dataset
=======================================================

Reproduces the main HUST experiment (Table 6 in the paper) with the
...
Usage
-----
    python train_hust.py <data_dir> [--seed 42] [--epochs 50]

Example
-------
    python train_hust.py "HUST motor multimodal dataset" --seed 42 --epochs 50
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train SCGN on HUST dataset")
    parser.add_argument("data_dir", type=str, help="Path to HUST dataset root directory")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--epochs", type=int, default=50, help="Max training epochs")
    parser.add_argument("--patience", type=int, default=10, help="Early-stopping patience")
    parser.add_argument("--lr", type=float, default=1e-3, help="Initial learning rate")
    parser.add_argument("--train_ratio", type=float, default=0.20, help="Training ratio (default 0.20 for HUST)")
    parser.add_argument("--device", type=str, default="auto", help="cuda / cpu / auto")
    parser.add_argument("--vibration_channel", type=str, default="z", choices=["x", "y", "z", "rms"],
                        help="Vibration channel to use")
    ...
    # Load dataset
    # ------------------------------------------------------------------
    print("\n[1/4] Loading HUST dataset ...")
    dataset = HUSTDataset(
        args.data_dir,
        seg_len=4096,
        overlap=0.0,
        speeds="all",
        vibration_channel=args.vibration_channel,
    )
    train_idx, val_idx, test_idx = split_hust_dataset(
    ...
    fft_vib = build_hust_fft_features(dataset.vib_signals, norm="zscore")
    fft_aco = build_hust_fft_features(dataset.aco_signals, norm="zscore")
    ...
    model = SCGN(num_classes=6, d=64, gcn_layers=1, fft_dim=2048, mid_dropout=0.3).to(device)
