"""
train_pu.py — Training Script for Paderborn University (PU) Dataset
====================================================================

Reproduces the main PU experiment (Table 5 in the paper) with the
proposed SCGN model.  Uses the default hyper-parameters reported in
Sec. IV-B:

  - train / val / test split : 20% / 20% / 60%
...
Usage
-----
    python train_pu.py <data_dir> [--seed 42] [--epochs 50]
...
def main():
    parser = argparse.ArgumentParser(description="Train SCGN on PU dataset")
    parser.add_argument("data_dir", type=str, help="Path to PU dataset root directory")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--epochs", type=int, default=50, help="Max training epochs")
    parser.add_argument("--patience", type=int, default=10, help="Early-stopping patience")
    parser.add_argument("--lr", type=float, default=1e-3, help="Initial learning rate")
    parser.add_argument("--train_ratio", type=float, default=0.20, help="Training ratio (default 0.20 for PU)")
    ...
    # Load dataset
    # ------------------------------------------------------------------
    print("\n[1/4] Loading PU dataset ...")
    dataset = PUDataset(args.data_dir, use_cache=True)
    train_idx, val_idx, test_idx = split_pu_dataset(
        dataset, train_ratio=args.train_ratio, val_ratio=0.20, seed=args.seed
    )
    print(f"Split -> train={len(train_idx)}, val={len(val_idx)}, test={len(test_idx)}")

    # ------------------------------------------------------------------
    # Global Z-score for current (train-set μ/σ)
    # ------------------------------------------------------------------
    print("[2/4] Applying global Z-score to current signals ...")
    dataset.aco_signals = global_zscore_current(dataset.aco_signals, train_idx)
    # Recompute STFT after current normalization
    from scgn.dataset_pu import compute_stft_spectrogram as _compute_stft
    dataset.aco_specs = np.stack([
        _compute_stft(dataset.aco_signals[i])
    ...
    # ------------------------------------------------------------------
    print("[4/4] Building inductive KNN graph (k=10) ...")
    fft_vib = build_pu_fft_features(dataset.vib_signals, norm="zscore")
    fft_aco = build_pu_fft_features(dataset.aco_signals, norm="zscore")
    ...
    # ------------------------------------------------------------------
    print("[5/4] Initialising SCGN model ...")
    # PU uses 4096-point segments @ 16kHz → rfft yields 2049 bins, minus DC = 2048
    model = SCGN(num_classes=9, d=64, gcn_layers=1, fft_dim=2048, mid_dropout=0.3).to(device)
